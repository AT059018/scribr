    ...
    if(res && res.length > 0) {
      ...
      if(this.state.speakerLabels) {
        ...
        if(this.lastLen < results.length) {
          ...
          for(var i=this.lastLen;i<results.length;i++) {
            if(results[i].speaker == undefined) {
              break;
            }
            const transcript = results[i].alternatives[0].transcript
    ...
    return (
      <Dropzone onDropAccepted={this.handleUserFile} onDropRejected={this.handleUserFileRejection} maxSize={200 * 1024 * 1024} accept="audio/wav, audio/l16, audio/ogg, audio/flac, .wav, .ogg, .opus, .flac" disableClick={true} className="dropzone _container _container_large" activeClassName="dropzone-active" rejectClassName="dropzone-reject" ref={(node) => {
        ...
        <div className="flex setup">
          ...
          <div className="column">
            <p>Keywords to spot: <input onChange={this.handleKeywordsChange} id="keywords" placeholder="Type comma separated keywords here (optional)" className="base--input" value={this.state.keywords}/></p>
          </div>
        </div>
